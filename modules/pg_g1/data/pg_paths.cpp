...
String PG_Paths::add_ts_sfx(String file_path) {
	int dt = int(PG_S(OS)->get_unix_time());
	int tz_ofs = PG_S(OS)->get_time_zone_info().bias * 60;
	// DOC: Cast is to avoid warning "A sub-expression may overflow before being assigned to a wider type".
	Dictionary d = PG_S(Time)->get_date_dict_from_unix_time(static_cast<int64_t>(dt) + tz_ofs);
	return vformat("%s_%04d-%02d-%02d_%02dh%02dm%02ds.%s", file_path.get_basename(), d["year"], d["month"], d["day"], d["hour"], d["minute"], d["second"], file_path.get_extension());
}
...
#ifdef PG_GD_FNS
	ClassDB::bind_static_method("PG_Paths", D_METHOD("modules_dir_path"), &PG_Paths::modules_dir_path);

	ClassDB::bind_static_method("PG_Paths", D_METHOD("audio_ext"), &PG_Paths::audio_ext);

	ClassDB::bind_static_method("PG_Paths", D_METHOD("npcs_dir_path"), &PG_Paths::npcs_dir_path);
	ClassDB::bind_static_method("PG_Paths", D_METHOD("npc_file_path", "file_name", "file_ext", "npc_id"), &PG_Paths::npc_file_path);
	ClassDB::bind_static_method("PG_Paths", D_METHOD("npc_dialogues_file_path", "npc_id"), &PG_Paths::npc_dialogues_file_path);

	ClassDB::bind_static_method("PG_Paths", D_METHOD("file_path_with_new_ext", "file_path", "file_ext"), &PG_Paths::file_path_with_new_ext);

	ClassDB::bind_static_method("PG_Paths", D_METHOD("log_dir_path"), &PG_Paths::log_dir_path);
	ClassDB::bind_static_method("PG_Paths", D_METHOD("log_file_path"), &PG_Paths::log_file_path);
	ClassDB::bind_static_method("PG_Paths", D_METHOD("old_log_file_path", "dt"), &PG_Paths::old_log_file_path);

	ClassDB::bind_static_method("PG_Paths", D_METHOD("input_file_path", "profile_id"), &PG_Paths::input_file_path);

	ClassDB::bind_static_method("PG_Paths", D_METHOD("add_ts_sfx", "file_path"), &PG_Paths::add_ts_sfx);
#endif
}
